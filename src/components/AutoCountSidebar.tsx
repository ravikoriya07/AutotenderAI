...
export type AutoCountSidebarProps = {
  /** When false, panel should slide out (parent controls mount for exit animation). */
  open: boolean;
  onClose: () => void;
...
/**
 * Right overlay panel for Auto Count — Symbol options (rotation + confidence).
 * Styling matches the Quantity take-off left layers drawer (light theme).
 */
export function AutoCountSidebar({
  open,
  onClose,
  rotationInvariant,
  ...
  return (
    ...
    >
      <div className="flex shrink-0 items-center justify-between gap-2 border-b border-border/70 px-2.5 py-2 sm:px-3 sm:py-2.5">
        <h2 className="text-sm font-semibold text-foreground">Auto count</h2>
        <button
          type="button"
          onClick={onClose}
          className={cn(
          ...
          aria-label="Close auto count panel"
        >
          <X className="h-4 w-4" />
        </button>
      </div>
